...
const AuthPage = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!email) {
      uiToast({ variant: 'destructive', title: 'حقول مطلوبة', description: 'يرجى إدخال البريد الإلكتروني' });
      return;
    }
    ...
    if (!password) {
      uiToast({ variant: 'destructive', title: 'حقول مطلوبة', description: 'يرجى إدخال كلمة المرور' });
      return;
    }
    ...
    try {
      ...
      if (mode === 'signIn') {
        ...
        if (error) {
          ...
          uiToast({ variant: 'destructive', title: 'حدث خطأ', description: errorMessage });
    ...
    <div className="flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8">
        <div className="flex flex-col items-center">
          <Logo className="h-12 w-auto" /> {/* Reduced logo size */}
        ...
        <form className="mt-8 space-y-6" onSubmit={handleAuth}>
          ...
          <div className="text-center">
            ...
            >
              {mode === 'signIn' ? 'ليس لديك حساب؟ سجل الآن' : mode === 'signUp' ? 'لديك حساب بالفعل؟ تسجيل الدخول' : 'العودة إلى تسجيل الدخول'}
            </button>
          </div>
        </form>
      </div>
